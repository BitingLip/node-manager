...
Manages multiple SDXL pipelines, orchestrates workers, and handles complex
inference workflows including multi-stage generation and pipeline switching.
"""
...
class PipelineManager(BaseWorker):
    """
    Manages multiple SDXL pipelines and orchestrates complex workflows.
    
    Handles task queuing, pipeline switching, multi-stage generation,
    and resource management across different inference types.
    """
    
    def __init__(self, worker_id: str = "pipeline_manager", config: Optional[Dict[str, Any]] = None):
    ...
    async def cleanup(self) -> None:
        ...
        # Clean up workers
        if self.sdxl_worker:
            await self.sdxl_worker.cleanup()
        
        if self.model_loader:
            await self.model_loader.cleanup()
        
        self.logger.info("Pipeline manager cleaned up")
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            **base_status,
            ...
            "available_pipelines": self.list_available_pipelines()
        }
