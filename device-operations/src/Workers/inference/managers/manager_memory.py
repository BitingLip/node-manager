...
Migrated from inference/memory_optimizer.py
Provides memory optimization strategies and VRAM management for inference operations.
"""
...
class MemoryManager:
    """
    Memory optimization and management for inference operations.
    
    This manager provides memory optimization strategies and VRAM management
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.enable_sequential_offload = config.get("enable_sequential_offload", False)
        ...
        self.attention_slice_size = config.get("attention_slice_size")
        self.max_memory_gb = config.get("max_memory_gb")
    ...
    async def initialize(self) -> bool:
        ...
        self.logger.info(f"Memory manager initialized for {self.device}")
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "models_on_cpu": len(self.models_on_cpu),
            "current_model_on_gpu": self.current_model_on_gpu,
            "peak_memory_gb": self.peak_memory,
            "current_memory": self.get_memory_stats().__dict__
        }
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            # Move all models to CPU and clear references
            for model_name in list(self.models_on_cpu.keys()):
                del self.models_on_cpu[model_name]
            ...
            self.logger.error(f"Memory manager cleanup error: {e}")
...
class MemoryEfficiencyMonitor:
    ...
    def __init__(self, optimizer: MemoryManager):
