"""
VAE Manager for SDXL Workers System
==================================

Migrated from models/vae_manager.py
Specialized VAE model management and optimization.
"""
...
try:
    ...
    try:
        ...
        # No fallback - require real dependencies
        raise ImportError("VAE Manager requires diffusers library. Install with: pip install diffusers")
...
class VAEManager:
    ...
    async def load_vae(self, vae_data: Dict[str, Any]) -> Dict[str, Any]:
        """Load a VAE model."""
        try:
            name = vae_data.get("name", "default")
            model_path = vae_data.get("model_path", "")
            
            if not model_path:
                # Use default VAE
                return await self.load_default_vae(name)
            
            # Load specific VAE
            result = await self.load_vae_model(name, model_path)
            return {"vae_loaded": result is not None, "name": name}
        except Exception as e:
            logger.error(f"Failed to load VAE: {e}")
            return {"vae_loaded": False, "error": str(e)}
    
    async def load_default_vae(self, name: str) -> Dict[str, Any]:
        """Load default VAE model."""
        try:
            if name in self.default_vaes:
                model_path = self.default_vaes[name]
                result = await self.load_vae_model(name, model_path)
                return {"vae_loaded": result is not None, "name": name}
            return {"vae_loaded": False, "error": f"Unknown default VAE: {name}"}
        except Exception as e:
            logger.error(f"Failed to load default VAE: {e}")
            return {"vae_loaded": False, "error": str(e)}
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "loaded_vaes": list(self.loaded_vaes.keys()),
            "memory_usage_mb": self.performance_stats.get("memory_usage_mb", 0.0),
            "total_loads": self.performance_stats.get("total_loads", 0),
