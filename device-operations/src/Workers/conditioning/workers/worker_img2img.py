"""
Img2Img Worker for SDXL Workers System
=====================================

Migrated from conditioning/img2img.py
Worker for image-to-image conditioning and processing.
"""
...
class Img2ImgWorker:
    ...
    This worker handles image conditioning, strength control, and noise injection.
    """
    ...
    async def initialize(self) -> bool:
        """Initialize img2img worker."""
        try:
            self.logger.info("Initializing img2img worker...")
            self.processor = Img2ImgProcessor(self.device, self.dtype)
            self.initialized = True
            self.logger.info("Img2img worker initialized successfully")
            return True
        except Exception as e:
            self.logger.error(f"Img2img worker initialization failed: {e}")
            return False
    
    async def process_img2img(self, img2img_data: Dict[str, Any]) -> Dict[str, Any]:
        """Process image-to-image conditioning."""
        try:
            input_image = img2img_data.get("input_image")
            strength = img2img_data.get("strength", 0.8)
            target_size = img2img_data.get("target_size", (1024, 1024))
            
            if not input_image:
                return {"error": "No input image provided"}
            
            # Process the image
            processed_image = self.processor.preprocess_image(input_image, target_size)
            
            return {
                "processed_image": processed_image,
                "strength": strength,
                "target_size": target_size,
                "conditioning_type": "img2img"
            }
        except Exception as e:
            self.logger.error(f"Failed to process img2img conditioning: {e}")
            return {"error": str(e)}
    
    async def get_status(self) -> Dict[str, Any]:
        """Get img2img worker status."""
        return {
    ...
    async def cleanup(self) -> None:
        """Clean up img2img worker resources."""
        try:
            self.logger.info("Cleaning up img2img worker...")
            self.processor = None
            self.initialized = False
            self.logger.info("Img2img worker cleanup complete")
        except Exception as e:
            self.logger.error(f"Img2img worker cleanup error: {e}")
