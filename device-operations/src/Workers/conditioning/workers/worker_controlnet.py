...
Migrated from conditioning/controlnet.py
Worker for ControlNet conditioning implementation for guided image generation.
"""
...
class ControlNetWorker:
    """
    Worker for ControlNet conditioning implementation.
    
    This worker handles ControlNet-guided image generation with various conditioning types.
    """
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.error("ControlNet worker initialization failed: %s", e)
            return False
    
    async def process_controlnet(self, controlnet_data: Dict[str, Any]) -> Dict[str, Any]:
        """Process ControlNet conditioning."""
        try:
            control_type = controlnet_data.get("control_type", "canny")
            control_image = controlnet_data.get("control_image")
            conditioning_scale = controlnet_data.get("conditioning_scale", 1.0)
            
            if not control_image:
                return {"error": "No control image provided"}
            
            # Process control image based on type
            if control_type in self.processor.supported_types:
                processed_image = self.processor.supported_types[control_type](control_image)
                
                return {
                    "control_type": control_type,
                    "processed_image": processed_image,
                    "conditioning_scale": conditioning_scale,
                    "control_mode": controlnet_data.get("control_mode", "balanced")
                }
            else:
                return {"error": f"Unsupported control type: {control_type}"}
                
        except Exception as e:
            self.logger.error("Failed to process ControlNet conditioning: %s", e)
            return {"error": str(e)}
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_types": list(self.processor.supported_types.keys()) if hasattr(self, 'processor') else []
        }
