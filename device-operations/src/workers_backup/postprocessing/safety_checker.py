"""
Safety checker and NSFW filtering for SDXL generated images.
Provides content filtering and safety assessment capabilities.
"""
...
class SafetyChecker:
    ...
    def __init__(
        ...
        logger.info(f"Safety checker initialized (enabled: {enabled})")
    ...
    def filter_images(
        ...
        for i, (image, is_safe) in enumerate(zip(images, safe_flags)):
            if is_safe:
                ...
                logger.warning(f"Image {i} flagged as unsafe, replacing")
...
class SafetyManager:
    ...
    def __init__(
        ...
        logger.info(f"Safety manager initialized (NSFW filter: {enable_nsfw_filter}, strict: {strict_mode})")
    ...
    def process_images(
        ...
        logger.info(f"Processed {len(images)} images, {len(results['unsafe_indices'])} flagged")
        return results
    ...
    def _log_unsafe_image(self, index: int, score: float) -> None:
        ...
        logger.warning(f"Image {index} flagged as unsafe (score: {score:.3f})")
